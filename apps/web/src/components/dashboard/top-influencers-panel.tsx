...
type TopInfluencersPanelProps = {
  items: InfluencerSummary[];
  isFavorite: (username: string) => boolean;
  onToggleFavorite: (username: string) => void;
};

function FollowersBars({ items }: { items: InfluencerSummary[] }) {
  const top = items.slice(0, 8);
  const maxFollowers = Math.max(...top.map((item) => item.followers), 1);

  return (
    <div className="space-y-2">
      {top.map((item) => {
        const width = Math.max((item.followers / maxFollowers) * 100, 2);
        return (
          <div key={item.id || item.username}>
            ...
            <div className="h-2 rounded-full bg-slate-100">
              <div
                className="h-2 rounded-full bg-gradient-to-r from-[#6e3ff3] to-[#aa8ef9]"
                style={{ width: `${width}%` }}
              />
            </div>
          </div>
...
export function TopInfluencersPanel({ items, isFavorite, onToggleFavorite }: TopInfluencersPanelProps) {
  if (items.length === 0) {
    return (
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[1.3fr_1fr]">
      <table className="min-w-full border-collapse text-sm">
        <thead>
          <tr className="border-b border-[#eceef2] bg-[#f8fafc] text-left text-[11px] uppercase tracking-wide text-slate-500 sm:text-xs">
            ...
            <th className="hidden px-2 py-2 md:table-cell">Avg views</th>
        ...
        <tbody>
          {items.map((item, idx) => {
            ...
            return (
              ...
              >
                <td className="px-2 py-2 text-xs sm:text-sm">#{idx + 1}</td>
                <td className="px-2 py-2">
                  <button
                    ...
                    aria-label={
                      favorite
                        ? `Remove @${item.username} from favorites`
                        : `Add @${item.username} to favorites`
                    }
                  >
                ...
                <td className="hidden px-2 py-2 text-xs sm:text-sm md:table-cell">{formatNumber(item.average_views)}</td>
      ...
      <div className="rounded-xl border border-[#eceef2] bg-[#fafbff] p-3 sm:p-4">
        <h3 className="text-sm font-semibold text-slate-800">Followers distribution</h3>
        <p className="mb-3 text-xs text-slate-500">Top 8 influencer accounts</p>
        <FollowersBars items={items} />
      </div>
    </div>
  );
}
