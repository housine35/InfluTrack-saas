...
export function FavoritesPanel({ items, onToggleFavorite }: FavoritesPanelProps) {
  if (items.length === 0) {
    return (
      <p className="rounded-lg border border-[#eceef2] bg-[#fafbff] px-4 py-3 text-sm text-slate-600">
        Aucun favori pour l&apos;instant. Ajoute des influenceurs depuis Search DB ou Top Influencers.
      </p>
    );
  }
          ...
          <tr key={item.id || item.username} className="border-b border-[#f0f2f6] text-slate-700 hover:bg-[#fcfcfd]">
            <td className="px-2 py-2">
              <button
                ...
                className="rounded-md border border-amber-300 bg-amber-50 p-1.5 text-amber-600 transition hover:bg-amber-100"
                aria-label={`Remove @${item.username} from favorites`}
              >
